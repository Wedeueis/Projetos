...
bool Camera::setCameraProp(std::string key, int value) {
	if ( system(NULL) ){
		...
		std::stringstream ss;
		ss << "v4l2-ctl -d" << id << " -c " << key << "=" << value << " 2>/dev/null";
...
bool Camera::UpdateExposure() {
	...
	if( this->meanbrightLevel-this->exposureThreshold > 80 ) {
		setCameraProp(std::string("exposure_absolute"), minExposure);
	}else if(this->meanbrightLevel-this->exposureThreshold < - 80) {
		setCameraProp(std::string("exposure_absolute"), maxExposure);
	}
}
...
int Camera::getMaxExposure() { return maxExposure; }

void Camera::setMaxExposure(int value) {
	this-> maxExposure = value;
}

int Camera::getMinExposure() { return minExposure; }

void Camera::setMinExposure(int value) { 
	this-> minExposure = value;
}
