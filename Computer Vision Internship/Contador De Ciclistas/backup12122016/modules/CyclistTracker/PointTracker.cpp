...
void PointTracker::ZeroAbsolutePointNumber(void) {
	this->absolutePointNumber = 0;
}
...
void PointTracker::AddPoint(cv::Point &point, cv::Rect &rectangle) {
	...
	this->points[absolutePointNumber] = CyclistTrackedObject(point, rectangle,
													  absolutePointNumber, 0);
	this->absolutePointNumber++;
}
