...
void OCR::train(Mat TrainData, Mat classes, int nlayers){
    ...
    Ptr<ml::TrainData> trainDataPtr = ml::TrainData::create(TrainData,ml::ROW_SAMPLE,trainClasses,Mat(),Mat(), weights, Mat());
    //Learn classifier
    ann->train( trainDataPtr, 0);
    trained=true;
}
...
int OCR::classifyKnn(Mat f){
    int response = (int)knnClassifier->findNearest( f, K,Mat(),Mat(),Mat() );
    return response;
}
void OCR::trainKnn(Mat trainSamples, Mat trainClasses, int k){
    K=k;
    // learn classifier
    knnClassifier = ml::KNearest::create();
    Ptr<ml::TrainData> trainDataPtr = ml::TrainData::create(trainSamples, ml::ROW_SAMPLE, trainClasses, Mat(), Mat(), Mat());			
    //
    knnClassifier->train( trainDataPtr, 0 );
}
