...
def main():
    ...
    with tf.Session(config=config) as s:
        ...
        sample_c = np.zeros(shape=[model.sample_num, model.n_cat + model.n_cont])

        samples_cont = np.random.normal(loc=0.0, scale=1.0, size=model.sample_num)
        ...
        for step in range(train_step['global_step']):
            ...
            epsilon = np.random.normal(loc=0.0, scale=1.0, size=[model.batch_size, model.n_cont])
            ...
            if step % train_step['logging_interval'] == 0:
                ...
                for k in range(0, 10):
                    ...
                    samples = s.run(model.g, feed_dict={ model.x: sample_x,model.z: [sample_z[0]],model.c: sample_feed})
                    # Summary saver
                    ...
                    sample_dir = results['output'] + str(k) + 'train_{:08d}.png'.format(step)
