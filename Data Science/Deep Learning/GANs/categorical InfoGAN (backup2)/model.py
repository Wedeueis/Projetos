...
class InfoGAN:
	...
	def build_infogan(self):
		...
		logli_dist = tf.reduce_sum(-tf.log(cat_dist_prob+ 1e-8) * cat_dist_prob, 1)
		dist_h = tf.reduce_mean(logli_dist)

		logli_Qcx = tf.reduce_sum(-tf.log(c_fake + 1e-8) * c_fake * self.c, 1)
		cond_h = tf.reduce_mean(logli_Qcx)

		cat_error = self.lambd * (dist_h-cond_h)

		self.q_loss = - cat_error
		...
		tf.summary.scalar("q_loss", cat_error)
