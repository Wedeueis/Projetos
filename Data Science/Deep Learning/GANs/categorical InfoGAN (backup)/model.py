...
class InfoGAN:
	...
	def __init__(self, s, batch_size=64, input_height=28, input_width=28, input_channel=1,
			        ...
			        dfilters_dim=64, gfilters_dim=64, fc_unit=1024, n_categories=10, z_dim=62,
		...
		self.c = tf.placeholder(tf.float32, shape=[None, self.n_cat], name='c')  # (-1, 10)
	...
	def classifier(self, x, reuse=None):
		with tf.variable_scope("classifier", reuse=reuse):
			...
			logits = tf.layers.dense(x, units=self.n_cat, name='d-fc-2')
	...
	def build_infogan(self):
		...
		cat_loss = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(logits=c_fake_logits,
                                                                            labels=self.c))

		self.q_loss = cat_loss

		#self.g_loss += cat_loss
		#cond_h = tf.reduce_mean(tf.reduce_sum(-tf.log(c_fake + 1e-8) * self.c, 1))
		#h = tf.reduce_mean(-tf.reduce_sum(tf.log(self.c + 1e-8) * self.c, 1))
		#self.q_loss = cond_h + h
		...
		q_params = [v for v in t_vars if v.name.startswith('c') or v.name.startswith('d') or v.name.startswith('g')]
