...
class InfoGAN:
    ...
    def __init__(self, s, batch_size=64, input_height=28, input_width=28, input_channel=1,
                 ...
                 g_lr=8e-4, d_lr=8e-4, is_train=True):
    ...
    def generator(self, z, c, reuse=None):
        with tf.variable_scope("generator", reuse = tf.AUTO_REUSE):
            ...
            x = tf.layers.dense(x, units= self.fc_unit, activation=tf.nn.leaky_relu, name='g-fc-0')
            x = tf.layers.batch_normalization(x, training=True)
            ...
            x = tf.layers.conv2d_transpose(x, 128, 4, 2, activation=tf.nn.leaky_relu, padding='SAME', name='g-conv2d-0')
            x = tf.layers.batch_normalization(x, training=True)
    ...
    def build_infogan(self):
        ...
        q_H = tf.reduce_mean(self.lambd*(q_cat_loss + q_cont_loss))
        # infoGAN loss function: Loss = V(D,G) - lambda*L(G,Q)
        self.q_loss = tf.abs((self.g_loss - q_H))
        ...
        q_params = [v for v in t_vars if v.name.startswith('c')]
